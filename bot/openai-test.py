...
WELCOME_MESSAGE = (
    ...
    "новая функция транскрипция отправьте голосовое или перешлите и бот напишет вам транскрипцию\n"
    "транскрипция может быть не совсем точной"
)
...
# Путь к директории для временных аудиофайлов
temp_audio_folder = Path("./temp_audio")
if not temp_audio_folder.exists():
...
async def handle_voice_message(message: types.Message):
    ...
    audio_file_path = temp_audio_folder / f"{voice.file_id}.oga"

    # Скачивание файла по полученному пути
    await bot.download_file(file_path, destination=audio_file_path)
    ...
    await message.reply(f"Транскрипция: {transcription.text}")
